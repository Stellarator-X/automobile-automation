...
protected:
    ...
    vector<double> state; //Actual, Present State
    vector<vector<double>> linear_spaces_state; //Actual, all possible states
    vector<int> space_state; //Spaced, Present state
    vector<vector<int>> PossibleStates_space; //Spaced, Present state

    //Action Variables
    vector<double> action; //Actual, present taken action
    vector<vector<double>> linear_spaces_actions; //Actual, all possible actions
    vector<int> spaced_action; //Spaced, current
    vector<vector<int>> PossibleActions_space; //Spaced, all possible
    ...
    queue<pair<PVector, int>> Route; //Array of nodes to be traversed in order to reach the destination following the minimum length path
    PVector position;
    vehicle *self;

    float mass; //mass of the car  - required for force calculations

    vector<PVector> coordinates; // Coordinates of all the nodes
...
public:
    ...
    CarDriving();//Initialise action_space and observation_space
    ...
    std::tuple <vector<int>, float, bool> step(vector<int> _action);//Obs, Rewards, Done
    
    vector<vector<int>> getPossibleActions_space(){
        return PossibleActions_space;
    }

    vector<vector<int>> getPossibleStates_space(){
    ...
    vector<int> max_action(map<pair<vector<int>, vector<int>>, float> Q, vector<int> state);
};


CarDriving::CarDriving(){//Initialise action_space and observation_space
    //Initialising reward variables as 0
    max_episode_steps = 1000;
    //action_space = vector<vector<int>>(7); - Setting up the Action Space
    reward = 0;
    prev_reward = 0;
    
    mass  = 2500; //kg
    //Setting Up the State Space:
    vector<double> dist_nextnode = linDiscretise(0.0, 1.0, 100);
    vector<double> dist_middle = linDiscretise(0.0, 1.0, 10);
    vector<double> speedSpace = linDiscretise(0.0, 1.0, 50);
    ...
    //Action Space: 
    vector<double> acc = linDiscretise(-0.5, 1.0, 20);
    vector<double> steer = linDiscretise(-1.0 ,1.0, 20);
    vector<double> brake = linDiscretise(0.0, 1.0, 10);
...
vector<int> CarDriving::max_action(map<pair<vector<int>, vector<int>>, float> Q, vector<int> state){
    vector<float> values;
...
std::tuple< vector<int>, float, bool> CarDriving::step(vector<int> action){
    //Returns resultant state, reward, done

    float reward;
    vector<int> final_state;
    bool isDone = false;
    vector<double> _action;
